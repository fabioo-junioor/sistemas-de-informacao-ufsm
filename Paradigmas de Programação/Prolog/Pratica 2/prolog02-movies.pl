...
/*
movie(american_beauty, A).
movie(N, 2000).
movie(N, A), A < 2000.
movie(N, A), A > 1990.
actor(F, N, _), actor(K, N, _), F@>K.
actress(N, scarlett_johansson, _), director(N, D).
actor(_,N,_), director(_,N).
(actor(_,N,_); actress(_,N,_)), director(_,N).
actor(F,john_goodman,_), actor(F, jeff_bridges,_).
*/

/*
released_after(M, Y) :- movie(M, A), A > Y.
released_before(M, Y) :- movie(M, A), A < Y.
same_year(M1, M2) :- movie(M1, A1), movie(M2, A2), A1=:=A2.
co_star(A1, A2) :- (actor(N,A1,_), actress(N,A2,_));(actor(N,A1,_), actress(N,_,A2));
    				(actor(N,_,A1), actress(N,A2,_));(actor(N,_,A1), actress(N,_,A2)).
*/
